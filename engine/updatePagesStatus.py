...
class Mailer:
    ...
    def send_email(self, msg_subject, msg_content):
        ...
        sg = sendgrid.SendGridAPIClient(
            apikey=
            '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'
        )
...
def update_pages_status(url=None):
    db.connect()
    ...
    if url is None:
        ...
        get_last_pages_status_sql = """
            ...
            WHERE `pages_status`.`site_id`=`pages`.`id` AND `pages`.`url`='%s'
    ...
    if cursor.rowcount == 0:
        print "URL address of '%s' not found in database." % url
        exit(1)
    ...
    for row in cursor.fetchall():
        ...
        if check_if_working(code) != 0:
            ...
            if check_if_working(last_status_code) == 0:
                ...
                mailer.prepare_failure_msg_and_send(
                    row['url'], code, code_description,
            ...
            if check_if_working(last_status_code) != 0:
                ...
                #print "send email about working for site "+row['url_adress']
                mailer.prepare_success_msg_and_send(
                    row['url'], row['last_working_time'],
                    datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        ...
        try:
            ...
            #print "Updating status of %-30s with code %s completed." % (row['url_adress'], code)
            #print
        except mysql.connector.errors.Error as err:
...
if __name__ == '__main__':
    ...
    if not interval is None:
        ...
        if len(args) == 0:
            ...
            for url in args:
                print "Running checking status of website: '%s'" % url
                update_pages_status(url)
